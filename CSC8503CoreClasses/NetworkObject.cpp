...
bool NetworkObject::ReadDeltaPacket(DeltaPacket &p) {
	...
	fullOrientation.x += ((float)p.orientation[1]) / 127.0f;
	fullOrientation.x += ((float)p.orientation[2]) / 127.0f;
	fullOrientation.y += ((float)p.orientation[3]) / 127.0f;

	object.GetTransform().SetPosition(fullPos);
	object.GetTransform().SetOrientation(fullOrientation);	// was localorientation
...
bool NetworkObject::WriteDeltaPacket(GamePacket**p, int stateID) {
	...
	dp->pos[1] = (char)currentPos.x;
	dp->pos[2] = (char)currentPos.y;
...
bool NetworkObject::WriteFullPacket(GamePacket**p) {
	...
	fp->fullState.position = object.GetTransform().GetPosition(); // was world position
